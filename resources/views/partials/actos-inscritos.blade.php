                                        ...
                                        @if (count($usuariosInscritos) > 0)
                                            @foreach ($usuariosInscritos as $reg)
                                                <tr style="padding: 0px; margin: 0px;">
                                                    ...
                                                    <td width="*">
                                                        <button type="button" class="btn btn-danger" onclick="eliminarInscrito({{ $reg->id }})"><i class="fa fa-user-times fa-lg"></i></button>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        @else
...
@if ($estadoAccionInscrito ?? '')
    @php
        $class = ($estadoAccionInscrito == 'ok') ? 'text-bg-success' : 'text-bg-danger';
        $mensaje = ($estadoAccionInscrito == 'ok') ? 'Usuario desinscrito correctamente' : 'Error en la desinscripción del usuario';
    @endphp
