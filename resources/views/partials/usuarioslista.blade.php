    ...
    <table class="table" style="width: 70%;">
        <thead>
            <tr>
                ...
                <td><a href="{{ url('/views/admin/usuariosNuevo.php') }}"><button class="btn btn-success"><i class="fa fa-plus fa-lg"></i>&nbsp;Crear usuario</button></a></td>
            </tr>
        </thead>
                                ...
                                @if (count($personas) > 0)
                                    @foreach ($personas as $reg)
                                        <tr style="padding: 0px; margin: 0px;">
                                            ...
                                            <td width="*">
                                                <button class="btn btn-primary" onclick="editarUsuario({{ $reg->id }})"><i class="fa fa-edit fa-lg"></i></button>
                                                <button class="btn btn-danger" onclick="eliminarUsuario({{ $reg->id }})"><i class="fa fa-trash-o fa-lg"></i></button>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
...
@if ($estadoAccion = session('estadoAccion'))
    @php
        $class = '';
        $mensaje = '';
        $texto = '';
